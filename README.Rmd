...
We use a built-in *mouse-brain* dataset from the paper ... For RNA velocity, we need spliced and un-spliced count matrices.

```{r load data for RNA velocity}
data("pancreas")
SCim_for_velo <- SCimplify_for_velocity(pancreas$emat, 
...
Vel <- supercell_estimate_velocity(SCim_for_velo$emat, SCim_for_velo$nmat)
```
...
clusters <- supercell_assign(pancreas$meta$clusters, SCim_for_velo$membership)

# Set up the color scheme
library(RColorBrewer) 
library(scales)
N.clusters <- length(unique(clusters))
pal <- setNames(colorRampPalette(brewer.pal(8, name= "Set1"))(N.clusters), 
...
library(velocyto.R)
set.seed(3)
tSNE.velocity.plot(Vel, cell.colors = color, delta.norm = FALSE, perplexity = 50, pcount = 1, arrow.scale = 1.5, xlab = "tSNE1", ylab = "tSNE2")
```
