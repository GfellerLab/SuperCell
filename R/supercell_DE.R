...
supercell_FindMarkers <- function(ge, supercell_size, clusters, ident.1, ident.2 = NULL, genes.use = NULL,
                                  logfc.threshold = 0.25, min.pct = 0.1, seed = 12345, only.pos = FALSE){
  ...
  w.mean.1 <- apply(ge[genes.use, cell.idx.ident.1 ], 1, function(x){wtd.mean(x, weights = cell.weigth.1)})
  w.mean.2 <- apply(ge[genes.use, cell.idx.ident.2 ], 1, function(x){wtd.mean(x, weights = cell.weigth.2)})
  ## exp of normalized expression is used to compute av.logFC
  w.mean.exp.1 <- apply(ge[genes.use, cell.idx.ident.1 ], 1, function(x){wtd.mean(x = expm1(x = x), weights = cell.weigth.1)}) # take true expression value (as seurat does), not log-transformed
  w.mean.exp.2 <- apply(ge[genes.use, cell.idx.ident.2 ], 1, function(x){wtd.mean(x = expm1(x = x), weights = cell.weigth.2)})
...
supercell_FindAllMarkers <- function(ge, supercell_size, clusters, genes.use = NULL, logfc.threshold = 0.25,
                                     min.pct = 0.1, seed = 12345, only.pos = FALSE){
  res <- list()
