...
#   df <- data.frame(x = fe_x,
#                       y = fe_y,
#                       #identity = factor(clusters),
#                       size = supercell_size)
# 
# 
#   membership <- rep(1:length(supercell_size), df$size)
# 
#   crt   <- stats::cor.test(df$x[membership], df$y[membership])
#   return(crt)
# }
# 
# weightedCorr(fe_x,fe_y,supercell_size)
...
corSparse <- function(X, Y = NULL, cov = FALSE) {
  ...
  if (!is.null(x = Y)) {
    ...
    muY <- colMeans(x = Y)
    covmat <- ( as.matrix(x = Matrix::crossprod(x = X, y = Y)) - n * Matrix::tcrossprod(x = muX, y = muY) ) / (n-1)
    sdvecX <- sqrt( (colSums(x = X^2) - n*muX^2) / (n-1) )
    sdvecY <- sqrt( (colSums(x = Y^2) - n*muY^2) / (n-1) )
    cormat <- covmat / Matrix::tcrossprod(x = sdvecX, y = sdvecY)
  } else {		
    covmat <- ( as.matrix(Matrix::crossprod(x = X)) - n * Matrix::tcrossprod(x = muX) ) / (n-1)
    sdvec <- sqrt(x = diag(x = covmat))
    cormat <- covmat / Matrix::tcrossprod(x = sdvec)
  }
  if (cov) {
...
SuperCellLinkPeaks <- function(
  ...
  res <- mylapply(
    ...
    FUN = function(i) {
      ...
      if (sum(peak.use) < 2) {
        ...
        #weighted corr using metacell size
        membership <- rep(1:ncol(object), object$size)
        weighted.peak.access <-  as(peak.access[membership,],"CsparseMatrix")
        colnames(weighted.peak.access) <- colnames(weighted.peak.access)
        weighted.gene.expression <- as( gene.expression[membership,],"CsparseMatrix")
        colnames(weighted.gene.expression) <- colnames(gene.expression)
        
        
        coef.result <- cor_method(
          X = weighted.peak.access,
          Y = weighted.gene.expression
        )
        ...
        if (nrow(x = coef.result) == 0) {
          ...
          #weighted corr using metacell size
          weighted.bg.access <-  as(bg.access[membership,],"CsparseMatrix")
          colnames(weighted.bg.access) <- colnames(weighted.bg.access)
 
          
          bg.coef <- cor_method(
            X = weighted.bg.access,
            Y = weighted.gene.expression
          )
  ...
  links <- Signac:::LinksToGRanges(linkmat = coef.matrix, gene.coords = gene.coords.use)
  # add zscores
  ...
  z.lnk <- Signac:::LinksToGRanges(linkmat = z.matrix, gene.coords = gene.coords.use)
